...
export const checkAutoTranslateStatus = id => {
  return ajaxRequest.get(`/api/v1/News/TranslatePreCheck/${id}`);
};
...
export const switchAutoTranslateStatus = () => {
  return ajaxRequest.put(`/api/v1/News/SwitchAutoTranslateStatus`);
};
...
export const switchTipsModalStatus = () => {
  return ajaxRequest.put(`/api/v1/News/SwitchTipsStatus`);
};
...
export const autoTranslate = options => {
  if (options.type === 1) {
    return ajaxRequest.put(
      `/api/v1/News/AutoTranslate/${options.id}?overwrite=true`
    );
  }
  if (options.type === 2) {
    return ajaxRequest.put(
      `/api/v1/News/AutoTranslate/${options.id}?overwrite=false`
    );
  }
  if (options.type === 3) {
    return ajaxRequest.put(`/api/v1/News/AutoTranslate/${options.id}`);
  }
};
