<template>
  <div class="table-content">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <div class="handle-btn-wrap">
            <span class="edit-icon" @click="handleMove(scope.row)"></span>
            <span class="more-operate" @click="_handleShowMoreOperate($event)"></span>
    ...
    <ul class="operate-section" ref="operateSection">
      <li class="operate-item" v-for="(it, index) in operateList" :key="index">{{it}}</li>
    </ul>
...
export default {
  ...
  data() {
    return {
      multipleSelection: [],
      operateList: [{name: "移动", flag: 'move'}, {name: "复制", flag: copy} , {name: "下线", flag: }, "置顶", "删除"]
    };
  },
  methods: {
    changePageNum(page) {
    ...
    _handleShowMoreOperate(ev) {
      this.$refs.operateSection.style.left = ev.pageX - ev.offsetX + 11 + "px";
...
.table-content {
  ...
  .handle-btn-wrap {
    ...
    .edit-icon {
      ...
      width: 16px;
      height: 16px;
      background: url("../../../static/images/news_edit.png") no-repeat center
        center;
