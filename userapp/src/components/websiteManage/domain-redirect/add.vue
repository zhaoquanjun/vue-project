    ...
    <div class="domain-wrap">
      <el-form ref="ruleForm" @submit.native.prevent label-width="80px" :model="form">
        <el-form-item style="color:#262626" label="源域名:">
          <el-select v-model="form.sourceDomain" placeholder="请选择" :popper-append-to-body="false">
            <el-option v-for="(item,index) in domainList" :key="index" :label="item" :value="item"></el-option>
          </el-select>
          <div class="el-form-item__error" v-if="error.souceErrorTip">{{error.souceOnerrorText}}</div>
        </el-form-item>
...
export default {
  props: ["domainList", "sourceDomain", "_targetDomain", "isEditor", "id", "backupShow"],
  data() {
    return {
      disabled: false,
      error: {
  ...
  methods: {
    ...
    async editorDomain(option) {
      let { data, status } = await modifyDomainRedirection(option, this.id);
      if (status === 200) {
        this.$notify({
