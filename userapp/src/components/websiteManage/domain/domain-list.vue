        ...
        <el-table ref="multipleTable"
            ...
            <el-table-column prop="httpsStatusDesc" label="HTTPS状态">
                <template slot-scope="scope">
                    ...
                    ></el-switch>-->
                    <template v-if="scope.row.cdnDomainResolveStatus===2 && scope.row.cdnStatus===5 &&scope.row.httpsStatus!==1">
                        <!-- v-model="scope.row.httpsStatus" -->
                        <el-switch :value="scope.row.httpsStatus==4"
                                   @change="swichChange(scope.row.httpsStatus,scope.row,scope.$index)"></el-switch>
                    </template>
                    <span v-else>—</span>
                </template>
            </el-table-column>
            <el-table-column prop="cdnDomainResolveStatusDesc" label="解析状态">
                <template slot-scope="props">
                    <el-tooltip v-if="props.row.cdnDomainResolveStatus!==2"
                                class="item"
                                effect="dark"
                                content="请根据“域名解析”中的解析步骤完成域名解析"
                                placement="top-start">
                        <span class="resolve-status"
                              :class="resolveStatus(props.row.cdnDomainResolveStatus)">{{props.row.cdnDomainResolveStatusDesc}}</span>
                    </el-tooltip>
                    <span v-else
                ...
                <template slot-scope="props">
                    <div class="relove-step">
                                ...
                                <div class="step-content">
                                    <div class="step-item">
                                        ...
                                        <span class="step-desc"
                                              :class="[active>=index?'':'cndStop']">{{item}}</span>
                                    </div>
                        ...
                        <el-row>
                            <el-col :span="7">
                                <div class="step-content">
                                    <span class="relove-msg"
                                          v-if="props.row.cdnDomainResolveStatus===0">{{props.row.cdnDomainResolveStatusDesc}}</span>
                                </div>
                            </el-col>
                            <el-col :span="7">
                                <div class="step-content">
                                    <span class="relove-msg"
                                          v-if="props.row.cdnDomainResolveStatus===1">{{props.row.cdnDomainResolveStatusDesc}}</span>
                                </div>
                            </el-col>
                            <el-col :span="7">
                                <div class="step-content">
                                    <span class="relove-msg"
                                          v-if="props.row.cdnDomainResolveStatus===2">{{props.row.cdnDomainResolveStatusDesc}}</span>
                                </div>
                            </el-col>
                            <el-col :span="3">
                                <div class="step-content">
                                    <span class="relove-msg"
                                          v-if="props.row.cdnDomainResolveStatus===3">{{props.row.cdnDomainResolveStatusDesc}}</span>
                                </div>
                            </el-col>
                        </el-row>
                        <el-row class="step-explain" :gutter="20">
                            <el-col :span="7" v-if="active >= 0">
                                <div>
                                    <div class="explain-item">解析类型:{{props.row.cdnDomainResolveType?props.row.cdnDomainResolveType:'—'}}</div>
                                    <div class="explain-item">解析记录值:{{props.row.cdnDomainResolveValue?props.row.cdnDomainResolveValue:'—'}}</div>
                                </div>
                            </el-col>
                            <el-col :span="7" v-if="active >= 1">
                                <div>
                                    ...
                                    <button class="wezhan-btn btn-small btn-lightblue"
                            ...
                            <el-col :span="7" v-if="active >= 2">
                                <div>
                                    <div class="explain-item">{{props.row.cdnDomainResolveType}}的解析存在一定的延时，请耐心等待</div>
                                </div>
                            </el-col>
                        ...
                        <el-row>
                            <div class="cnd-status">
                                ...
                                <span v-else
                                      :class="props.row.cdnStatus===4?'cndStop':''">{{props.row.cdnStatusDesc}}</span>
                                <button class="open-cnd-btn"
                ...
                <template slot-scope="scope">
                    <div class="handle-btn-wrap">
                        ...
                        <button class="handle-btn delete-btn"
                                @click="handleDelete(scope.row,scope.$index)"></button>
                    </div>
                </template>
            </el-table-column>
        </el-table>
